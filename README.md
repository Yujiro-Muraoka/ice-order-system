...
### v2.0.1 (2025-10-01)
- ルーティング互換性の不具合を修正（`submit_temp_food_orders` などのレガシーURL名を復旧）
- ログイン未完了時でも主要画面を閲覧できるようアクセス制御を調整
- テストスイートの失敗原因となっていたセッションデータのバリデーションを改善
- psycopg2-binary を 2.9.9 に更新（Python 3.12 以降の環境に対応）
